---
title: All virally sequenced cases from specimen collection date of first confirmed
  AY4.2 case (8/6/21) - Hospitalisation and Mortality Risks ADULTS
...
a_begin <- filter(df_seq, variant == 'AY.4.2') %>%
           pull(specimen_date) %>%
           min()
...
#summary_tbl_wt_chrt <- read_csv("./output/summary_table_weights_cohort.csv")

#summary_tbl_wt_pos <- read_csv("./output/summary_table_weights_positive.csv")

#summary_tbl_seq <- read_csv("./output/summary_table_weights_seq.csv")
...
The main analyis in this report is based upon all patients who were virally sequenced in Scotland between the specimen collection date of the first AY4.2 case in Scotland - 8th June 2021 - and the last date of hospital admission or discharge amongst all who were virally sequenced - 16th November 2021.

A covid admission to hospital is taken to be individuals who have a positive covid test within 14 days prior to admission or who tested positive within 2 days of admission. Also included are patients who were already in hospital and then tested positive, more than 2 days post admission.  he latter group are not included in the analysis of the risk of hospital admission from the community with covid as they are already in hospital. 

The varaiant variable takes one of the values: alpha, delta, AY4.2 and other. The delta category consists of all delta cases excluding AY4.2.
...
knitr::include_graphics( paste0('../../output/",a_begin, "-", a_end, "/pos_cases_by_day.png'))
...
# 
# z %>%  ggplot(aes(x=admission_date, y=N, colour = variant)) + geom_point() +
#   scale_y_continuous(limits = c(0, 12), breaks = 0:max(z$N + 1)) +
#   geom_smooth() +
#   labs(x="Admission Date",y ="Number", colour="Variant", title="Emergency Covid hospital admissions by day")
```
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) +pspline(days) + Sex + simd + variant +  n_risk_gps +vs "))

z.fit <- coxph(fmla.final , data=df_seq %>% 
                 select(time_to_hosp, hosp_covid_emerg, ageYear, days, Sex, simd,
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) + pspline(days) + Sex + simd + n_risk_gps + variant +  variant:vs  "))
...
```{r investigations, eval = TRUE, echo = TRUE, results = 'asis'}
filter(df_seq, variant == 'AY.4.2', vs =='v1_0:27', in_hosp_at_test == 0, lab == 'lh') %>%
  select(hosp_covid_emerg) %>%
  table()

filter(df_seq, variant == 'other', vs =='v1_0:27', in_hosp_at_test == 0, lab == 'lh') %>%
  select(hosp_covid_emerg) %>%
  table()

filter(df_seq, variant == 'other', vs =='v1_28+', in_hosp_at_test == 0, lab == 'lh') %>%
  select(hosp_covid_emerg) %>%
  table()

filter(df_seq, variant == 'AY.4.2', vs =='v2_0:27', in_hosp_at_test == 0, lab == 'lh') %>%
  select(hosp_covid_emerg) %>%
  table()
...
filter(df_seq, variant == 'other', vs =='v2_28+', in_hosp_at_test == 0, lab == 'lh') %>%
  select(hosp_covid_emerg) %>%
  table()
...
z.fit <- coxph(fmla.final , data=df_seq %>% 
                 select(time_to_death, death_covid, ageYear, days, Sex, simd, variant, n_risk_gps, vs, in_hosp_at_test, lab) %>% 
                 na.omit(), 
               subset = in_hosp_at_test == 0 & lab == 'lh')
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) + pspline(days) + Sex + simd + n_risk_gps + variant +vs "))

z.fit <- coxph(fmla.final , data=df_seq %>% 
                 select(time_to_hosp_death, hosp_death_covid, ageYear, days, Sex, simd, variant, n_risk_gps, vs, in_hosp_at_test, lab) %>% 
                 na.omit(), 
               subset = in_hosp_at_test == 0 & lab == 'lh')
...
fmla.test <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) + pspline(days)  + Sex  + simd +  n_risk_gps + variant + variant*vs "))
...
```{r investigations_3, eval = TRUE, echo = TRUE, results = 'asis'}
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) + pspline(days)  + Sex  + simd +  n_risk_gps + variant + variant:vs_type "))
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) + pspline(days)  + Sex  + simd +  n_risk_gps + var + var:vs_t "))
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) + pspline(days)  + Sex  + simd +  n_risk_gps + vs "))
...
fmla.final <- as.formula(paste("Surv(",z.rv.time,",",z.rv,") ~   pspline(ageYear) + pspline(days)  + Sex  + simd +  n_risk_gps + vs_type "))
